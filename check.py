...
def check_pairs(folder_path):
    # Get all files
    files = os.listdir("/home/camila/yolo_detc_seg/RealSense-Camera-Based-Pavement-Segmentation-and-Centre-Point-Generation-Dynamic-Object-Detection/coco/images/train")
    
    # Separate images and xmls
    images = {f.rsplit('.', 1)[0] for f in files if f.lower().endswith(('.jpg', '.png', '.jpeg'))}
    xmls = {f.rsplit('.', 1)[0] for f in files if f.endswith('.txt')}
