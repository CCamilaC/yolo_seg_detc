...
def get_orientation_and_center(contour):
    """
    Retorna o ângulo e o centro do retângulo mínimo que envolve o contorno.
    """
    (cx, cy), (w, h), angle = cv2.minAreaRect(contour)
    if w < h:
        angle += 90
    return angle, np.array([cx, cy])


def merge_if_same_orientation(contours, angle_threshold=12, max_distance=200):
    """
    Une contornos próximos e com mesma orientação.
    """
    if len(contours) <= 1:
        return contours
    ...
    for i in range(len(contours)):
        ...
        base_angle, center_i = get_orientation_and_center(contours[i])
        if center_i is None:
            continue
        ...
        for j in range(i + 1, len(contours)):
            ...
            angle_j, center_j = get_orientation_and_center(contours[j])
            if center_j is None:
                continue
...
try:
    while True:
        ...
        # Convertendo para HSV e criando máscara do verde
        ...
        # -----------------------------
        # Detecção via Canny + bounding box
        # -----------------------------
        ...
        for cnt in filtered_contours:
            angle, center = get_orientation_and_center(cnt)
            if center is not None:
                cx, cy = int(center[0]), int(center[1])
                coords.append((cx, cy))
                cv2.circle(output, (cx, cy), 6, (0, 0, 255), -1)

                # Determinar ponta do cone
                pts = cnt.reshape(-1, 2)
                ...
                # Calcular ângulo da ponta em relação ao centro
                dx = cone_tip[0] - cx
                dy = cone_tip[1] - cy
